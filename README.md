...
└── web_simulation.py       # [NEW] Web Monitor for Headless Server (Ubuntu)
```
...
### Prerequisites
* **Hardware Target:** Intel Core i5-2400 running Ubuntu Server Headless.
* **Camera:** 720p USB Webcam (plugged directly into the server, e.g., `/dev/video0`).
* **GPU Training:** RTX 5090 (or similar High-VRAM GPU) to rapidly train A4 printable high-res patches via GA Batching.
* **Note on Profiling:** This project runs strictly on **bare-metal python** without Docker. Virtualization arbitrarily shifts CPU load calculations, making physical DoS architecture metrics unrealistic.
...
python -m venv venv
source venv\Scripts\activate #venv/bin/activate  # Windows: venv\Scripts\activate
...
```bash
python test_physical_dos.py --cam 0 --patch outputs/sponge_patch_A4_g100_p100.png
```

### 3. Headless Web Simulation (Ubuntu Server)
Designed exclusively for headless environments (like the Core i5 Ubuntu Server). This runs the 720p hardware camera stream, superimposes the A4 adversarial patch, and broadcasts the live NMS bottleneck/FPS breakdown over an HTTP dashboard.
```bash
python web_simulation.py
```
*Access the Live HUD at: `http://<SERVER_IP>:5000`*
...
## 📊 Performance Metrics (Intel Core i5-2400 - Ubuntu Server)
Empirical evidence of the physical attack's impact on an x86 architecture standard edge server via 720p HD Webcam:

| System State | Raw Boxes ($N_{active}$) | IoU Operations/Frame | Core i5-2400 Load | 720p FPS Impact |
|---|---|---|---|---|
| Clean Stream | ~ 15 - 25 | ~ 300 | 15% - 20% | 30 FPS (Smooth) |
| Under Attack | 300+ (Max) | > 45,000 | 100% (Overload) | < 2 FPS (Frozen/Lag) |
